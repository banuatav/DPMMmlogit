dpmm_mnl_gibbs_wPrior = function(y, y_p, X_con, X_cat, X_p, Dummy, Dummy_p, PriorPar, MCMCpar, I = NULL){
  ################################## General ######################################
  ...
  if(is.null(I)){
    ...
    # Hyperparameters
    if(is.null(PriorPar$InitialHyperPar)){
      ...
      mu_mu = PriorPar$InitialHyperPar$mu_mu
      upsilon_mu = PriorPar$InitialHyperPar$upsilon_mu
      nu_mu = PriorPar$nu_mu
      Sigma_mu  = PriorPar$InitialHyperPar$Sigma_mu
      
      nu_Sigma = PriorPar$nu_Sigma
      upsilon_Sigma = PriorPar$InitialHyperPar$upsilon_Sigma
      
      a0              = PriorPar$InitialHyperPar$a0
      
      upsilonbeta    = PriorPar$InitialHyperPar$upsilonbeta
      nubeta         = PriorPar$nubeta
      betabar        = PriorPar$InitialHyperPar$betabar
      B              = PriorPar$InitialHyperPar$B
    }
    
    # Cluster assignment, assigmnent initialized as all belong to cluster 1 if no other specification
    ...
    # Initial Parameters 
    if(is.null(PriorPar$InitialThetaStar)){
      for(k in 1:K){
      ...
      SigmaStar = PriorPar$InitialThetaStar$SigmaStar
      muStar    = PriorPar$InitialThetaStar$muStar
      betaStar  = PriorPar$InitialThetaStar$betaStar
      piStar    = PriorPar$InitialThetaStar$piStar
      alpha     = PriorPar$InitialThetaStar$alpha
    }
